# -*- coding: utf-8 -*-
"""botidenhancedclipped2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1MJYuE5Ue9aueg36-a1gPY959I9LlVIuk
"""
...
class EnhancedSportsTrack:
    ...
    def __init__(self, track_id, detection, frame_idx):
        ...
        # Initialize missing occlusion attributes
    ...
    def _get_center(self):
        """Get center point of bbox with bounds checking"""
        try:
...
class EnhancedSportsTracker:
    ...
    def __init__(self, reid_model_path=None, max_tracks=25):
        ...
        # IMPROVED team clustering parameters
        self.team_clusters = {}
        self.cluster_update_interval = 20  # More frequent updates (was 30)

        # Enhanced clustering parameters
        self.min_samples_per_team = 2  # Reduced from 3
        self.clustering_eps = 0.4  # Slightly higher eps for more separation (was 0.3)
        self.min_track_age_for_clustering = 5  # Only use stable tracks

        # Team assignment history for stability
        self.team_assignment_history = defaultdict(list)
        ...
        # CRITICAL: Initialize tracks and related attributes
        self.tracks = []  # This was missing!
    ...
    def _clip_bbox_to_frame(self, bbox, frame_shape):
        ...
        try:
            ...
            bbox[0] = max(0, min(bbox[0], w - 1))  # x1
            bbox[1] = max(0, min(bbox[1], h - 1))  # y1
            bbox[2] = max(bbox[0] + 1, min(bbox[2], w))  # x2
            bbox[3] = max(bbox[1] + 1, min(bbox[3], h))  # y2
    ...
    def update(self, detections, frame=None):
        """Enhanced tracking update with bbox clipping"""
        start_time = time.time()
        self.frame_count += 1

        try:
            # Get frame shape for clipping
            frame_shape = frame.shape if frame is not None else None

            # Filter detections
            valid_detections = self._filter_detections(detections, frame)

            # CLIP ALL DETECTION BBOXES
            for det in valid_detections:
                det['bbox'] = self._clip_bbox_to_frame(det['bbox'], frame_shape)
            ...
            # Predict track positions and clip them
            for track in self.tracks:
                predicted_bbox = track.predict()
                track.bbox = self._clip_bbox_to_frame(predicted_bbox, frame_shape)
...
class SportsVideoProcessor:
    ...
    def _visualize(self, frame, tracks, detections, frame_idx):
        """Enhanced visualization with proper bounding box clipping"""
        ...
        # Draw detections (transparent) with clipping
        for det in detections:
            try:
                bbox = det['bbox']
                if bbox is not None and len(bbox) >= 4:
                    # Clip detection boxes to frame boundaries
        ...
        # Draw tracks with team information and proper clipping
        for track in tracks:
            try:
                ...
                try:
                    # PROPER CLIPPING: Keep boxes within frame boundaries
                    x1 = max(0, min(int(float(bbox[0])), frame_width - 1))
                    y1 = max(0, min(int(float(bbox[1])), frame_height - 1))
                    x2 = max(x1 + 1, min(int(float(bbox[2])), frame_width))
                    y2 = max(y1 + 1, min(int(float(bbox[3])), frame_height))
                ...
                # Draw bounding box (now properly clipped)
                ...
                # Draw label background with clipping
                try:
                    (text_w, text_h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)

                    # Clip label position to frame
                    label_x1 = max(0, x1)
                    label_y1 = max(text_h + 5, y1)
                    label_x2 = min(frame_width, label_x1 + text_w + 5)
                    label_y2 = max(0, label_y1 - text_h - 5)

                    cv2.rectangle(vis_frame, (label_x1, label_y2), (label_x2, label_y1), color, -1)

                    # Draw label text (clipped)
                    text_x = max(2, label_x1 + 2)
                    text_y = max(text_h, label_y1 - 5)
                    cv2.putText(vis_frame, label, (text_x, text_y),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
                except:
                    pass

                # Draw trajectory with clipping
                if hasattr(track, 'position_history') and len(track.position_history) > 1:
                    try:
                        points = []
                        for pos in track.position_history:
                            # Clip trajectory points to frame
                            clipped_x = max(0, min(int(pos[0]), frame_width - 1))
                            clipped_y = max(0, min(int(pos[1]), frame_height - 1))
